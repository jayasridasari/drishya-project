...
// Admin: update user
 ...
 exports.deleteUser = async (req,res,next) => {
  try {
    const { id } = req.params;
    if(id===req.user.id) return res.status(400).json({ error:'Cannot delete your own acco
 ...
 exports.updateProfile = async (req,res,next) => {
  try {
    ...
    let idx=1;
    if(email) {
      const conflict = await query('SELECT 1 FROM users WHERE email=$1 AND id!=$2', [emai
...
await query(
 ...
 await query('UPDATE users SET password_hash=$1, updated_at=NOW() WHERE id=$2',[newHas
